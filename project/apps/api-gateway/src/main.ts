...
async function bootstrap() {
  ...
  const host = configService.get(`${ConfigEnvironment.API_GATEWAY}.${PostConfigEnum.HOST}`);
  const port = configService.get(`${ConfigEnvironment.API_GATEWAY}.${PostConfigEnum.PORT}`);
